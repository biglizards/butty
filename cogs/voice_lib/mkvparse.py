# modified from https://github.com/vi/mkvparse/
# (most modifications are the automatic pep-8 by my ide though)
...
def read_matroska_number(f, unmodified=False, signed=False):
    ...
    if unmodified and signed:
        raise Exception("Contradictory arguments")
    first_byte = f.read(1)
    if first_byte == "":
        raise StopIteration
    r = ord(first_byte)
...
# lynx -width=10000 -dump http://matroska.org/technical/specs/index.html | sed 's/not 0/not0/g; s/> 0/>0/g;
# s/Sampling Frequency/SamplingFrequency/g' | awk '{print $1 " " $3 " " $8}' | grep '\[..\]' | perl -ne '/(\S+) (\S+)
#  (.)/; $name=$1; $id=$2; $type=$3; $id=~s/\[|\]//g; %types = (m=>"EET.MASTER", u=>"EET.UNSIGNED", i=>"EET.SIGNED",
# 8=>"EET.TEXTU", s=>"EET.TEXTA", b=>"EET.BINARY", f=>"EET.FLOAT", d=>"EET.DATE"); $t=$types{$type}; next unless $t;
# $t="EET.JUST_GO_ON" if $name eq "Segment" or $name eq "Cluster"; print "\t0x$id: ($t, \"$name\"),\n";'
...
def read_ebml_element_tree(f, total_size):
    ...
    while total_size > 0:
        ...
        if size == -1:
            sys.stderr.write(
                "mkvparse: Element %x without size? Damaged data? Skipping %d bytes\n" % (id_, size))
            f.read(total_size)
            break
        if size > total_size:
            sys.stderr.write(
                "mkvparse: Element %x with size %d? Damaged data? Skipping %d bytes\n" % (id_, size, total_size))
            f.read(total_size)
            break
        type_ = EET.BINARY
        name = "unknown_%x" % id_
...
class MatroskaHandler:
    ...
    @staticmethod
    def begin_handling_ebml_element(id_, name, type_, headersize, datasize):
        return type_
...
def handle_block(buffer, handler, cluster_timecode, timecode_scale=1000000, duration=None,
    ...
    if laceflags == 0x00:  # no lacing
        buf = buffer[pos:]
        handler.frame(tracknum, block_timecode, header_removal_prefix + buf, 0, duration, f_keyframe, f_invisible,
                      f_discardable)
        return
    ...
    for i in lengths:
        ...
        pos += i
        handler.frame(tracknum, block_timecode, header_removal_prefix + buf, more_laced_frames, duration, f_keyframe,
                      f_invisible, f_discardable)
        more_laced_frames -= 1
...
def mkvparse(f, handler):
    ...
    while f:
        ...
        # cluster contents:
            ...
            data = f.read(size)
            handle_block(data, handler, current_cluster_timecode, timecode_scale, None,
                         header_removal_headers_for_tracks)
